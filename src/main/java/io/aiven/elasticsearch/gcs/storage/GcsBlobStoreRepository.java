...
public class GcsBlobStoreRepository extends BlobStoreRepository {
    ...
    public static final String TYPE = "gcs-encrypted";
    ...
    private static final String ENCRYPTION_KEY_FILE = "key.enc";

    private final GcsSettingsProvider gcsSettingsProvider;

    private final BlobPath basePath;

    public GcsBlobStoreRepository(final RepositoryMetaData metadata,
    ...
    static BlobPath buildBasePath(final RepositoryMetaData repositoryMetaData) {
        final String basePath = BASE_PATH.get(repositoryMetaData.settings());
        BlobPath blobPath = BlobPath.cleanPath();
    ...
    private SecretKey createOrRestoreEncryptionKey(final Storage storage) throws IOException {
        final var encryptedKeyBlobId = BlobId.of(bucketName(), basePath().buildAsString() + ENCRYPTION_KEY_FILE);
        ...
        if (Objects.nonNull(encryptedKeyBlob) && encryptedKeyBlob.exists()) {
            ...
            encryptionKey = gcsSettingsProvider.encryptionKeyProvider().decryptKey(encryptedKeyBlob.getContent());
        } else {
            ...
            final var encryptedEncryptionKey = gcsSettingsProvider.encryptionKeyProvider().encryptKey(encryptionKey);
            storage.create(BlobInfo.newBuilder(encryptedKeyBlobId).build(), encryptedEncryptionKey);
        }
